...
from myapp.api.seriaizers import JobAdvert_Serializer, JobApplication_Serializer
...
class JobPosting(APIView):
    ...
    def get(self, request, format=None):
        ...
        adverts = JobAdvert.objects.prefetch_related("Job_applications").all().annotate(applicant_count=Count('Job_applications')).order_by("status","-Job_applicant_count", "-date_added")
...
class JobAdvertDetail(APIView):
    ...
    def get_object(self, advert_id):
        try:
            return JobAdvert.objects.prefetch_related("Job_applications").get(id=advert_id)
    ...
    def get(self, request, advert_id, format=None):
        ...
        serializer = JobAdvert_Serializer(posting)
        return Response(serializer.data, status=status.HTTP_200_OK)


    @swagger_auto_schema(method="put", request_body=JobAdvert_Serializer())
    @action(methods=["put"], detail=True)
    def put(self, request, advert_id, format=None):
        """Allows logged in users to edit the details of a job advert"""
        
        posting = self.get_object(advert_id)
        serializer = JobAdvert_Serializer(posting, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
...
@swagger_auto_schema(method="post", request_body=JobApplication_Serializer())
@api_view(["POST"])
@permission_classes([AllowAny])
def job_application(request):
    ...
    if request.method == 'POST':
        serializer = JobApplication_Serializer(data=request.data)
...
class JobApplicationDetail(APIView):
    ...
    def get(self, request, application_id, format=None):
        ...
        serializer = JobApplication_Serializer(application)
        return Response(serializer.data, status=status.HTTP_200_OK)
